...
if (!$run) {
    renderForm($expectedKey !== '', $perRun, implode(',', $sources), $personsPerKey);
    exit;
}
...
.log{background:#111820;border:1px solid #243041;border-radius:10px;padding:12px 14px;min-height:60vh;white-space:pre-wrap;word-break:break-word}
...
echo '<div class="meta">Live progress — each person prints when complete. Source: <b>'
    ...
    . '</b> · batch: <b>' . $perRun . '</b> · rotate key every <b>' . $personsPerKey . '</b> persons</div>';
...
try {
    ...
    out('info', 'Connected. TMDB keys loaded: ' . $keyCount);
    out('info', sprintf(
...
function renderForm(bool $keyConfigured, int $limit, string $source, int $ppk): void
{
    header('Content-Type: text/html; charset=utf-8');
    $limit = (int) $limit;
    $ppk = (int) $ppk;
...
.warn{color:#ffd166;font-size:13px;margin-bottom:12px}
    ...
    echo '<h1>Person / Credits sync</h1>';
    echo '<p>Browser runner — 6 TMDB keys cycle: every N persons → next key. Live progress in page.</p>';
    if (!$keyConfigured) {
    ...
    echo 'Reads TMDB_API_KEYS from server .env. Default: 10 persons → next key → cycle.<br>';
    echo 'HTTP 429 આવે તો તરત next key પર switch.</div>';
    echo '</div></body></html>';
}
