...
/**
 * Browser runner: drain media_sync_queue for tv + backfill.
 *
 * Open in browser:
 *   https://app.myappworld.in/tmdb/api/sync-tv-backfill.php
 *   http://localhost/tmdb/api/sync-tv-backfill.php
 *
 ...
 *
 * Then enter API key and click Start — each completed TV show prints live.
 * Does not enqueue related people (use sync-person-credits.php for that).
 */
...
const DEFAULT_PER_RUN = 20;
const STUCK_MINUTES = 30;
const DEFAULT_SHOWS_PER_KEY = 5;
...
$perRun = max(1, min(100, $perRun));
$showsPerKey = (int) ($_GET['spk'] ?? $_POST['spk'] ?? DEFAULT_SHOWS_PER_KEY);
$showsPerKey = max(1, min(50, $showsPerKey));
$tab = (int) ($_GET['tab'] ?? $_POST['tab'] ?? 0);
if ($tab < 0) {
    $tab = 0;
}
$source = strtolower(trim((string) ($_GET['source'] ?? $_POST['source'] ?? 'backfill')));
if ($source === '' || !preg_match('/^[a-z0-9_,]+$/', $source)) {
    $source = 'backfill';
}
$sources = array_values(array_filter(array_map('trim', explode(',', $source))));
$allowedSources = ['backfill', 'changes', 'discover'];
$sources = array_values(array_filter($sources, static fn(string $s): bool => in_array($s, $allowedSources, true)));
if ($sources === []) {
    $sources = ['backfill'];
}

if (!$run) {
    renderForm($expectedKey !== '', $perRun, implode(',', $sources), $showsPerKey, $tab);
    exit;
}
...
echo '<title>TV/Backfill Sync</title>';
echo '<style>
...
echo '<h1>TV / Backfill sync</h1>';
echo '<div class="meta">Live progress — each show prints when complete. Source: <b>'
    . htmlspecialchars(implode(',', $sources), ENT_QUOTES, 'UTF-8')
    . '</b> · batch: <b>' . $perRun . '</b> · rotate key every <b>' . $showsPerKey
    . '</b> shows · keys: <b>' . htmlspecialchars($keySliceLabel, ENT_QUOTES, 'UTF-8') . '</b></div>';
...
try {
    ...
    out('info', sprintf(
        'Key rotation: every %d shows → next key (cycle of %d). On 429 auto-switch to next key.',
        $showsPerKey,
        $keyCount
    ));
    ...
    if ($counts['pending'] === 0 && $counts['running'] === 0) {
        out('ok', 'Nothing pending. TV/backfill queue is complete.');
    ...
    while ($processed < $perRun) {
        ...
        $slot = (int) floor(($globalOffset + $processed) / $showsPerKey) % max(1, $keyCount);
        if ($slot !== $activeSlot) {
            $tmdb->pinToIndex($slot);
            $activeSlot = $slot;
            out('warn', sprintf(
                '→ Using %s for next %d show(s)',
                $tmdb->keyLabel($slot),
                $showsPerKey
            ));
        }
        ...
        out('dim', sprintf(
            '[%d/%d] queue#%d tv tmdb_id=%d source=%s attempt=%d %s …',
            $processed,
            $perRun,
            $qid,
            $tmdbId,
        ...
        try {
            ...
            $action = $writer->syncTv($tmdb, $tmdbId);
            markDone($pdo, $qid);
            $pdo->commit();
            $done++;
            $ms = (int) round((microtime(true) - $t0) * 1000);
            out('ok', sprintf(
                '✓ DONE  tv %d (%s) in %dms via %s  | batch done=%d fail=%d',
                $tmdbId,
                $action,
                $ms,
                $tmdb->keyLabel(),
            ...
            $failed++;
            out('err', sprintf(
                '✗ FAIL  tv %d — %s  | batch done=%d fail=%d',
                $tmdbId,
                $e->getMessage(),
                $done,
                $failed
    ...
    out('ok', 'ALL COMPLETE — tv/backfill pending=0');
    echo '</div><p class="meta"><a href="?">Back</a></p></div></body></html>';
} catch (Throwable $e) {
...
function renderForm(bool $keyConfigured, int $limit, string $source, int $spk, int $tab = 0): void
{
    header('Content-Type: text/html; charset=utf-8');
    $limit = (int) $limit;
    $spk = (int) $spk;
    $tab = max(0, (int) $tab);
    echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>TV/Backfill Sync</title>';
    echo '<style>
...
.warn{color:#ffd166;font-size:13px;margin-bottom:12px}
    ...
    echo '<h1>TV / Backfill sync</h1>';
    echo '<p>Browser runner for incomplete TV seasons/episodes. Optional <code>tab</code> uses 2 TMDB keys each. Live progress in page.</p>';
    if (!$keyConfigured) {
    ...
    echo '<input type="number" name="limit" min="1" max="100" value="' . $limit . '">';
    echo '<label>Shows per TMDB key (then rotate)</label>';
    echo '<input type="number" name="spk" min="1" max="50" value="' . $spk . '">';
    echo '<label>Source</label>';
    echo '<select name="source">';
    echo '<option value="backfill"' . ($source === 'backfill' ? ' selected' : '') . '>backfill only</option>';
    echo '<option value="backfill,changes"' . ($source === 'backfill,changes' ? ' selected' : '') . '>backfill + changes</option>';
    echo '<option value="backfill,discover,changes"' . ($source === 'backfill,discover,changes' ? ' selected' : '') . '>backfill + discover + changes</option>';
    ...
    echo 'Enqueue incomplete TV from admin-sync.html first, then start this runner.<br>';
    echo 'Open 4 browser tabs with tab=1..4 (8 keys in .env). Queue uses SKIP LOCKED — no double claim.<br>';
    echo 'Each TV show fetches every season — keep batch limit 10–20. Related people are not enqueued here.</div>';
    echo '</div></body></html>';
}
...
function queueCounts(PDO $db, array $sources): array
{
    ...
    $stmt = $db->prepare(
        "SELECT status, COUNT(*) AS cnt
         ...
         WHERE media_type = 'tv' AND source IN ({$in})
         GROUP BY status"
    );
